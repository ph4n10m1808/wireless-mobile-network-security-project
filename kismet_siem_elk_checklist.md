...
- [x] **1. Chuẩn bị Hạ tầng & Thiết lập Môi trường**
  ...
  - [x] **1.2. Giả lập Sóng Vô tuyến ảo**
    ...
    - [x] 1.2.2. Kiểm tra driver bằng lệnh `iw dev` (Đảm bảo xuất hiện các interface ảo từ `wlan0` đến `wlan7`)
...
- [x] **2. Triển khai & Tùy biến Hạ tầng SIEM ELK Stack**
  - [x] **2.1. Cấu hình Docker Compose (`docker-compose.yml`)**
    ...
    - [x] 2.1.2. Cấu hình volume mount log cho Logstash để map `/var/log/virtual-wips`, `/var/log/virtual-network`, và thư mục log Kismet thô `/var/log/kismet` từ Host vào Container
    - [x] 2.1.3. Khởi tạo khóa mã hóa Kibana (`ENCRYPTION_KEY`) bằng script `generate_key.sh`
  - [x] **2.2. Cấu hình Pipeline Logstash phân tách**
    - [x] 2.2.1. Thiết lập cấu hình chính `logstash.conf` nhận log giả lập mạng (`network-events.json`) và log WIDS chuẩn hóa (`wips-alerts.json`)
    - [x] 2.2.2. Thiết lập cấu hình riêng biệt `kismet.conf` nhận log trực tiếp từ card mạng/Kismet thô (`kismet_direct`)
    - [x] 2.2.3. Cấu hình trường lọc tránh xung đột tag và trùng lặp bản ghi (Double Ingestion)
...
- [x] **3. Cài đặt & Triển khai Kismet WIDS (Tầng Giám sát Vô tuyến)**
  ...
  - [x] **3.2. Cấu hình Card Giám sát Monitor Mode**
    - [x] 3.2.1. Lựa chọn card mạng ảo trống (ví dụ `wlan7`) làm anten nghe trộm sóng ảo
    - [x] 3.2.2. Chuyển card `wlan7` sang Monitor mode (`sudo ip link set wlan7 down && sudo iw dev wlan7 set type monitor && sudo ip link set wlan7 up`)
    - [x] 3.2.3. Kiểm tra lại trạng thái monitor mode bằng lệnh `iw dev wlan7`
  - [x] **3.3. Cấu hình & Chạy Kismet**
    - [x] 3.3.1. Chạy Kismet chỉ định bắt gói trên card monitor ảo (`sudo kismet -c wlan7 --no-sqlite`)
    - [x] 3.3.2. Truy cập giao diện Web UI Kismet tại `http://localhost:2501` để kiểm tra quét sóng thành công
  - [x] **3.4. Chạy Cầu nối Đồng bộ API (`kismet_to_elk.py`)**
...
- [x] **4. Triển khai Module Phản ứng Chủ động (WIPS Active Response)**
  - [x] **4.1. Thiết lập Engine Phản ứng (`wips_elk_containment_simulator.py`)**
    - [x] 4.1.1. Chạy daemon giám sát real-time file log chuẩn hóa đầu ra của Kismet Bridge
    - [x] 4.1.2. Đảm bảo hàm phân tích cú pháp log JSON hoạt động bình thường
  - [x] **4.2. Thực thi Ngăn chặn & Cô lập**
    ...
    - [x] 4.2.2. Tự động ghi MAC/IP vi phạm vào file blacklist tường lửa (`simulated_blacklist.txt` trên Host)
    - [x] 4.2.3. Ghi lại lịch sử hoạt động ngăn chặn vào nhật ký cách ly `/var/log/virtual-wips/active-response.log`

- [ ] **5. Cấu hình Dashboard & Phân tích Cảnh báo (Kibana SIEM)**
  - [ ] **5.1. Tạo Data Views trên Kibana**
    - [ ] 5.1.1. Tạo Data View `wids-alerts-*` kết nối tới Elasticsearch (cho cả log Bridge và log Kismet trực tiếp)
    - [ ] 5.1.2. Tạo Data View `network-events-*` kết nối tới Elasticsearch
    - [ ] 5.1.3. Kiểm tra ánh xạ kiểu dữ liệu (đảm bảo các trường như `event_type.keyword` đã sẵn sàng)
  - [ ] **5.2. Thiết kế giao diện Dashboard tập trung**
    ...
    - [ ] 5.2.2. Widget 2: Biểu đồ cột thể hiện Top SSIDs/BSSIDs bị giả mạo do Kismet bắt được
    - [ ] 5.2.3. Widget 3: Biểu đồ tròn phân tích tỷ lệ các loại tấn công (Deauth Flood, Rogue AP, Evil Twin)
    - [ ] 5.2.4. Widget 4: Bảng theo dõi tương quan sâu (MAC vô tuyến do Kismet bắt $\rightarrow$ IP DHCP cấp $\rightarrow$ Port scan trên tường lửa)
...
- [x] **6. Tấn công Thực nghiệm, Kiểm thử & Đánh giá (Live Hacking & Testing)**
  ...
  - [x] **6.2. Kiểm thử Kịch bản Deauthentication Flood thật**
    - [x] 6.2.1. Mở terminal tấn công, sử dụng `aireplay-ng -0 150 -a <BSSID> -c <Client_MAC> wlan7` gửi gói deauth flood thực vào sóng vô tuyến ảo
  ...
  - [x] **6.3. Kiểm thử Tương quan Hệ thống & Phản ứng**
    - [x] 6.3.1. Kiểm tra log Kismet Bridge nhận sự kiện $\rightarrow$ Đẩy lên Logstash $\rightarrow$ Kibana hiển thị cảnh báo
    - [x] 6.3.2. Xác nhận daemon Active Response tự động bắt được alert của Kismet và ghi MAC/IP kẻ tấn công vào `simulated_blacklist.txt` cách ly thành công
...
- [ ] **7. Hoàn thiện Đồ án & Chuẩn bị Báo cáo Bảo vệ**
  - [ ] **7.1. Biên soạn Slide Thuyết trình**
    ...
    - [ ] 7.1.2. Slide Kiến trúc: Sơ đồ Mermaid luồng dữ liệu Kismet WIDS -> Bridge API -> Logstash -> ES -> Kibana SIEM
  ...
  - [ ] **7.2. Chuẩn bị Kịch bản Demo thực tế trước Hội đồng**
    - [ ] 7.2.1. Thiết lập sẵn 6 Cửa sổ Terminal tương ứng với 6 luồng hoạt động
